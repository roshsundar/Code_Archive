...
void loop(){
  ...
  //Low Pass Filter
  FxAng = xAng * alpha + ( FxAng * (1.0 - alpha));
  FyAng = yAng * alpha + ( FyAng * (1.0 - alpha));
  FzAng = zAng * alpha + ( FzAng * (1.0 - alpha));
  
  x= RAD_TO_DEG * (atan2(-FyAng, -FzAng)+PI); 
  y= RAD_TO_DEG * (atan2(-FxAng, -FzAng)+PI); 
  z= RAD_TO_DEG * (atan2(-FyAng, -FxAng)+PI);
